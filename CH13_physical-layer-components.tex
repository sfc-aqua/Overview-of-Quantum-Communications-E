...
%The Bell states are four entangled states, and they can be written in this form in the computational basis. \ket{\Phi^+} is a superposition of  \ket{00}, and  \ket{11}. So is \ket{\Phi^-}, but with a negative phase between the two, and this (follow pointer) is the expression for \ket{\Psi^+}, and \ket{\Psi^-}.
...
We start with the usual initial state, a superposition of \ket{0} and \ket{1}, but after application of the Hadamard gate we get a new state, which we are denoting \ket{\psi'}, given in this form (follow pointer). A Hadamard applied to \ket{0} gives us a \ket{+} state, whereas a Hadamard applied to \ket{1} gives us the \ket{-} state. If we just expand the \ket{+} state and the \ket{-} state again in the computational basis, we arrive at this form \rdv{?} for our state \ket{\Psi^+} \rdv{? is that right? Should it be \ket{\psi'}?}. Now, because we are measuring in the $Z$ basis, it's easy to read out the probabilities corresponding to outcomes $+1$ and $-1$.
...
So, why is that? Well, the clue is in the probabilities. If we look at the probability of the $+1$ outcome in this scheme (see pointer), where we are measuring the $X$ basis directly, then it's given by the inner product of \ket{\psi} and the \ket{+} state modulus squared. If we do it in our other scheme where we are using the Hadamard followed by a z-measurement, then it's given by the corresponding expression here (see pointer), now it's the inner product between the state \ket{\psi'}, so this is our initial state after we apply Hadamard to it, and the inner product is with a \ket{0} state.

And then, what we get is \ket{\psi} times Hadamard times \ket{0}. If we are comparing these two probabilities (follow pointer), we see that this \ket{\psi} state has an inner product with \ket{+}, and \ket{+} is just written as Hadamard applied to a \ket{0} state. So that's what we are doing here (top right red box circuit).
...
\rdv{reworked; Michal, please check}
...
Hans J. Briegel, Wolfgang Dür, Juan I. Cirac, Peter Zoller, Quantum repeaters: The role of imperfect local operations in quantum communication, \emph{Physical Review Letters} 81, 5932, 1998.
