...
\begin{tikzpicture}[
  ...
  \begin{scope}
    ...
    \foreach \i [evaluate={\R=\i*\lambd;}] in {1,...,\N}{
      \ifodd\i
        \draw[line width=0.8] (0,\a/2)++(\ang:\R) arc (\ang:-\ang:\R);
        \draw[line width=0.8] (0,-\a/2)++(\ang:\R) arc (\ang:-\ang:\R);
      \else
        \draw[line width=0.1] (0,\a/2)++(\ang:\R) arc (\ang:-\ang:\R);
        \draw[line width=0.1] (0,-\a/2)++(\ang:\R) arc (\ang:-\ang:\R);
      \fi
    }
  \end{scope}
  ...
  \foreach \i [evaluate={\x=-\i*\lambd;}] in {0,...,5}{
    \ifodd\i
      \draw[line width=0.8] (\x,-\h/2) -- (\x,\h/2);
    \else
      \draw[line width=0.1] (\x,-\h/2) -- (\x,\h/2);
    \fi
  }
  ...
  \begin{scope}[shift={(1.08*\L,0)}]
    ...
    \foreach \i [evaluate={\n=0.5*\i;\yn=\L/sqrt((\a/(2*\lambd*\n))^2-1);
      ...
      \ifodd\i % if even
        \fill[white] (0,{-\yn-0.1}) rectangle++ (\A,0.2); % to fill seams
        \fill[white] (0,{ \yn-0.1}) rectangle++ (\A,0.2); % to fill seams
      \fi
    }
