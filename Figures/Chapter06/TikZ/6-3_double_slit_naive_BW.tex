...
\begin{tikzpicture}[
    ...
    declare function={
      ...
      ynode(\n,\dn,\lam,\a) = (2*\n*\dn+\dn^2)*\lam/(2*\f);
      intensity_single(\y,\a) = exp(-(\y+\a/2)^2)/2;
      intensity_total(\y,\a) = (exp(-(\y-\a/2)^2)+exp(-(\y+\a/2)^2))/2;
    }
  ]
  ...
  \begin{scope}
    ...
    \foreach \i [evaluate={\R=\i*\lambd;}] in {1,...,\N}{
      \ifodd\i
        \filldraw[fill=black,rotate around={\Angles[\i]:(0,\a/2)}] (\R,\a/2) circle (0.08);
    \else
        \filldraw[fill=black,rotate around={\Angles[\i]:(0,\a/2)}] (\R,-\a/2) circle (0.08);
      \fi
    }
  ...
  \foreach \i [evaluate={\x=-\i*\lambd;}] in {0,...,5}{
    \ifodd\i
      \filldraw[fill=black] (\x,0) circle (0.08);
  ...
  \begin{scope}[shift={(1.08*\L,0)}]
    ...
    \path[left color=black,right color=black, middle color=white, shading angle=0] (0,-4) rectangle (\A,4);
