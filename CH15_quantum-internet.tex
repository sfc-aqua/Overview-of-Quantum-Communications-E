...
\rrr Yes! So in fact, that's what we're going to do next, is we're going to actually use them with the classical Internet. So the first task is for the quantum network to make those keys, turning quantum states into classical bits, and then it's going to share those bits with a couple of boxes that are connected to an IP network that are called IPsec gateways, and there will be one of those at each end. Have you heard of IPsec\index{IPsec}?
...
\mmm The fourth thing, we have to think more in terms of networking and how to handle things like routing, how to handle things like multiplexing when there is contention for resources, and how to manage all of these things including security.
